...
class EnergyTracker:
    ...
    async def find_optimal_execution_window(
        self, region: str, duration_hours: int, max_delay_hours: int = 24
    ) -> Optional[datetime]:
        """Find the optimal time window to execute a job based on energy prices"""
        try:
            # Fetch price forecasts for the next max_delay_hours
            forecasts = await self._get_price_forecasts(
                region, max_delay_hours
            )
            ...
            logger.info(
                f"Found optimal window starting at {start_time} "
