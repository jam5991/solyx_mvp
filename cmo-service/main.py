...
def get_scheduler(db: Session = Depends(get_db)) -> Scheduler:
    ...
    if os.getenv("VAST_API_KEY"):
        scheduler.add_provider(
            VastAIProvider(api_key=os.getenv("VAST_API_KEY")), "vastai"
        )

    if os.getenv("LAMBDA_API_KEY"):
        scheduler.add_provider(
            LambdaLabsProvider(api_key=os.getenv("LAMBDA_API_KEY")), "lambda"
        )
    ...
    for name, provider in scheduler.providers.items():
        try:
            ...
            providers[name] = {
                ...
                "price_range": {
                    "min": min(gpu.price_per_hour for gpu in gpus)
                    if gpus
                    else None,
                    "max": max(gpu.price_per_hour for gpu in gpus)
                    if gpus
                    else None,
                },
            }
        except Exception as e:
            providers[name] = {"status": "error", "error": str(e)}
    ...
    try:
        if provider:
            ...
            return {
                ...
                "price_stats": {
                    "min": min(g.price_per_hour for g in gpus)
                    if gpus
                    else None,
                    "max": max(g.price_per_hour for g in gpus)
                    if gpus
                    else None,
                    "avg": sum(g.price_per_hour for g in gpus) / len(gpus)
                    if gpus
                    else None,
                },
            }
            ...
            for prov_name, provider in scheduler.providers.items():
                ...
                provider_stats[prov_name] = {
                    ...
                    "by_type": {
                        gpu_type: len(
                            [g for g in gpus if g.gpu_type == gpu_type]
                        )
                        for gpu_type in set(g.gpu_type for g in gpus)
                    },
                    "price_stats": {
            ...
            return {"total_gpus": total_gpus, "providers": provider_stats}
...
async def readiness_check(scheduler: Scheduler = Depends(get_scheduler)):
    ...
    if not scheduler.providers:
        raise HTTPException(
            status_code=503, detail="No GPU providers configured"
        )
    return {"status": "ready"}
...
async def submit_job(
    ...
    try:
        ...
        if not gpu:
            raise HTTPException(
                status_code=404, detail="No GPU found matching requirements"
            )
...
async def get_job_status(
    job_id: str, scheduler: Scheduler = Depends(get_scheduler)
):
