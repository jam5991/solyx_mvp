...
class RayJobExecutor:
    ...
    def execute(self, job_spec: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            buffer_handler.setFormatter(logging.Formatter('%(asctime)s - %(message)s'))
            ...
            return {
                "status": "failed", 
                "error": str(e),
                "logs": log_buffer.getvalue() if 'log_buffer' in locals() else ""
            }
...
class RayJobManager:
    ...
    async def submit_job(self, job_spec: Dict[str, Any], gpu_instance: GPUInstance) -> Dict[str, Any]:
        ...
        try:
            ...
            for job_id, (executor, future) in list(self.active_jobs.items()):
                try:
                    ...
                    logger.error(f"Error cancelling Ray task for job {job_id}: {e}")
            ...
            try:
                ...
                if allocation_count > 5:
                    ...
                    allocation_to_delete = session.query(GPUAllocation.id)\
                    ...
                    if allocation_to_delete:
                        session.query(GPUAllocation)\
                            .filter(GPUAllocation.id.in_([id[0] for id in allocation_to_delete]))\
                            .delete(synchronize_session=False)
                        logger.info(f"Deleted {len(allocation_to_delete)} old allocation records")
                ...
                session.query(GPUAllocation)\
                    .filter(GPUAllocation.released_at.is_(None))\
