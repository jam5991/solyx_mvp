    ...
    async def list_available_gpus(self) -> List[GPUInstance]:
        ...
        try:
            ...
            async with aiohttp.ClientSession(headers=self.headers) as session:
                async with session.get(f"{self.base_url}/instances") as response:
                    ...
                    for instance in data.get('data', []):
                        try:
                            gpu = GPUInstance(
                                ...
                                gpu_type=instance['instance_type']['gpu_description'],
                                memory_gb=instance['instance_type']['specs']['memory_gib'],
                                price_per_hour=instance['instance_type']['price_cents_per_hour'] / 100,
                                region=instance['region']['name'],
                                available=True
                            )
                            instances.append(gpu)
                            self.logger.info(f"Added instance: {gpu.instance_id} - {gpu.gpu_type}")
